...
- **🚨 严重修复：强制路由模式无法清除** - 自动清除失败的强制模式
  - **问题**：系统缓存了 `forcedMode=v3`，即使删除插件重装也无法清除
  - **后果**：V3 路由失败但系统不尝试 V2，导致交易持续失败
  - **修复**：智能故障恢复机制
    - 检测到强制 V3 模式失败时，自动清除强制模式并尝试 V2
    - 检测到强制 V2 模式失败时，自动清除强制模式并尝试 V3
    - 清除后比较两者输出，选择最优路由
  - **日志增强**：
    - 显示路由提示信息（forcedMode、lastMode、routerAddress）
    - 明确标识强制模式状态
    - 记录强制模式清除过程
  - **影响**：彻底解决强制模式导致的路由锁定问题
  - **容错性**：确保系统始终能够尝试所有可用路由
